...
class DiagGaussianPdType(PdType):
    ...
    ## Newly added
    def dyn_pdfromlatent(self, latent_vector, init_scale=1.0, init_bias=0.0, index=None):
        mean = fc(latent_vector, 'dyn%s'%index, self.size, init_scale=init_scale, init_bias=init_bias)
        logstd = tf.get_variable(name='dyn%s'%index +'/logstd', shape=[1, self.size], initializer=tf.zeros_initializer())
        pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
        return self.pdfromflat(pdparam), mean
